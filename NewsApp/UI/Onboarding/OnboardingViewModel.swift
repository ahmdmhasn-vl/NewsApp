...
class OnboardingViewModel: ObservableObject {
    ...
    func handleCategorySelection(index : Int){
        ...
        }else if(selectedNewsCategories.count == 0){
            selectedNewsCategories.append(categories[index])
        }else{
            if ValidationUtils.isValidCategoriesCount(selectedCategories: selectedNewsCategories) {
                selectedNewsCategories.append(categories[index])
    ...
    func validate() -> Bool {
        ...
        if !ValidationUtils.isValidCountry(slectedCountry: selectedCountry) {
            isValid = false
