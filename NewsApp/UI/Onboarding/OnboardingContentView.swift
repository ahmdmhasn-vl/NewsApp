    ...
    var body: some View {
        VStack() {
            ...
            if viewModel.showNewsCategoryError {
                Text("Please check at least on category").frame(minWidth: 0, maxWidth: .infinity,alignment: .leading).padding(.leading,20).padding(.vertical,5).foregroundColor(Color.red).font(.system(size: 13))
            }
            HStack {
                Button("Submit"){
